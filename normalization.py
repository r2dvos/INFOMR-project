...
def compute_scaling(bounds: np.ndarray) -> float:
    max_bound = max([abs(bounds[0][0] - bounds[1][0]), abs(bounds[0][1] - bounds[1][1]), abs(bounds[0][2] - bounds[1][2])])
    return max_bound

def normalize_shape(shape: trimesh.Trimesh) -> trimesh.Trimesh:
    # Position normalization
    barycenter = compute_barycenter(shape.faces, shape.vertices, shape.triangles_center)
...
if __name__ == "__main__":
    ...
    if mode == "-f":
        ...
        normalized_shape.export(objFile)
        ...
        for root, _, files in os.walk(sys.argv[2]):
            for file in files:
                if file.endswith('.obj'):
                    ...
                    normalized_shape.export(full_path)
    elif mode == "-b":
        root = tk.Tk()
        root.withdraw()
        objFile = filedialog.askopenfilename(initialdir = "../../ShapeDatabase_INFOMR_copy")
        shape = trimesh.load(objFile)
        print(compute_barycenter(shape.faces, shape.vertices, shape.triangles_center))
    else:
        print(help)
